...
=======
        ...
        // 6. CRUCE DE CLAVES
        console.log('📋 [DIAG] 5. CRUCE DE CLAVES:');
        ...
        clavesLocales.forEach(({ clave, row }) => {
            const match = clavesSheets.get(clave);
            
            if (match) {
                foundCount++;
                console.log(`| ${clave.padEnd(20)} | Found     | ${String(match.fila).padEnd(4)} | ${String(match.idDetalle).padEnd(10)} | OK |`);
            } else {
                notFoundCount++;
                console.log(`| ${clave.padEnd(20)} | Not Found | -    | -          | Ver análisis abajo |`);
            }
        });
        ...
        // 7. ANÁLISIS DE NOT FOUND
        if (notFoundCount > 0) {
            console.log('📋 [DIAG] 6. ANÁLISIS DE NOT FOUND:');
            ...
            clavesLocales.forEach(({ clave, row }) => {
                ...
                if (!match) {
                    ...
                    console.log(`   Artículo local: "${row.articulo}"`);
                    
                    // Buscar artículos similares en Sheets
                    const articuloLocal = String(row.articulo).trim();
                    console.log('   Artículos similares en Sheets:');
                    
                    let encontroSimilar = false;
                    detalles06c76584Sheets.forEach(item => {
                        const articuloSheet = String(item.articulo || '');
                        
                        // Verificar si son similares (mismo número base)
                        if (articuloSheet.includes(articuloLocal) || 
                            articuloLocal.includes(articuloSheet.replace(/[,.]/g, '')) ||
                            articuloSheet.includes('e+') ||
                            articuloSheet.endsWith('.0')) {
                            
                            const claveSheet = normalizarClave(item.idPresupuesto, item.articulo);
                            console.log(`     - Raw: "${item.articuloRaw}" → Normalizado: "${claveSheet}"`);
                            encontroSimilar = true;
                        }
                    });
                    
                    if (!encontroSimilar) {
                        console.log('     - No se encontraron artículos similares');
                    }
                }
            });
        }
