...
async function obtenerPackMapping(codigoPadre) {
    ...
    try {
        ...
        try {
            console.log(`🔍 [PACK] Buscando descripción del hijo: ${articulo.pack_hijo_codigo}`);
            ...
            if (hijoResponse.ok) {
                ...
                console.log(`📊 [PACK] Respuesta hijo (intento 1):`, hijoResult);
                
                if (hijoResult.success && hijoResult.data && hijoResult.data.length > 0) {
                    const hijoData = hijoResult.data.find(art => art.articulo_numero === articulo.pack_hijo_codigo);
                    if (hijoData) {
                        hijoDescripcion = hijoData.descripcion || hijoData.nombre || hijoData.articulo_descripcion;
                        console.log(`✅ [PACK] Descripción hijo encontrada (intento 1): "${hijoDescripcion}"`);
                    }
                }
            }
            
            if (!hijoDescripcion) {
                console.log(`⚠️ [PACK] Intento 1 falló, probando intento 2...`);
                ...
                if (hijoResponse2.ok) {
                    ...
                    if (hijoResult2.success && hijoResult2.data && hijoResult2.data.length > 0) {
                        const hijoData2 = hijoResult2.data.find(art => art.articulo_numero === articulo.pack_hijo_codigo);
                        if (hijoData2) {
                            hijoDescripcion = hijoData2.descripcion || hijoData2.nombre || hijoData2.articulo_descripcion;
                            console.log(`✅ [PACK] Descripción hijo encontrada (intento 2): "${hijoDescripcion}"`);
                        }
                    }
                }
            }
            ...
            if (hijoDescripcion) {
                ...
                console.log(`⚠️ [PACK] No se pudo obtener descripción del hijo ${articulo.pack_hijo_codigo}, usando fallback`);
            }
            
        } catch (hijoError) {
            console.error(`❌ [PACK] Error al obtener nombre del hijo:`, hijoError);
        }
        
        const packMapping = {
            articulo_produccion_codigo: codigoPadre,
            articulo_kilo_codigo: articulo.pack_hijo_codigo,
...
console.log('✅ [EXPAND] Módulo de filas expandibles cargado (v17.3 - robusto sin hardcodes)');
