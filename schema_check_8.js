...
content = content.replace(
    /card\.className = `tarjeta-ingrediente \$\{\(ingrediente\.stock_total <= 0\) \? 'con-stock-cero' : ''\}`;/g,
...
content = content.replace(
    /card\.className = `tarjeta-ingrediente \$\{\(ingrediente\.stock_actual <= 0 && ingrediente\.stock_potencial <= 0\) \? 'con-stock-cero' : ''\}`;/g,
...
content = content.replace(
    /<input type="checkbox" class="checkbox-ajuste" data-id="\$\{ingredienteIdReal\}" onchange="window\.toggleSeleccionAjuste\(this\)">/g,
