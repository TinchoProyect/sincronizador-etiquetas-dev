...
const obtenerPresupuestos = async (req, res) => {
    try {
        ...
        console.log('[PRESUPUESTOS-BACK] 🔄 TRANSFORMANDO DATOS AL FORMATO FRONTEND...');
        
        const datosParaFrontend = result.rows.map(row => ({
            // Campos que espera el frontend
            categoria: row.agente || 'Sin categoría',
            concepto: `${row.id_cliente} - ${row.id_presupuesto_ext}`,
            monto: row.valor_total_estimado || 0,
            fecha_registro: row.fecha,
            fecha_sincronizacion: new Date().toISOString(), // Fecha actual como última sync
            ...
            cliente_nuevo_id: row.cliente_nuevo_id,
            punto_entrega: row.punto_entrega,
            descuento: row.descuento,
            hoja_nombre: row.hoja_nombre,
            hoja_url: row.hoja_url,
            total_articulos: row.total_articulos,
            cantidad_total: row.cantidad_total,
            valor_total_estimado: row.valor_total_estimado
        }));
        
        console.log(`[PRESUPUESTOS-BACK] ✅ DATOS TRANSFORMADOS: ${datosParaFrontend.length} registros`);
        console.log('[PRESUPUESTOS-BACK] Ejemplo de dato transformado:', datosParaFrontend[0]);
        
        res.json({
            success: true,
            data: datosParaFrontend,
            total: totalRecords,
            pagination: {
                total: totalRecords,
                limit: parseInt(limit),
...
/**
 * Obtener estadísticas de presupuestos CON LOGS - FORMATO FRONTEND
 */
const obtenerEstadisticas = async (req, res) => {
    try {
        console.log('[PRESUPUESTOS-BACK] 📊 CALCULANDO ESTADÍSTICAS PARA FRONTEND...');
        ...
        const syncQuery = `
            SELECT MAX(fecha_sync) as ultima_sync
            FROM presupuestos_sync_log 
            WHERE exitoso = true
        `;
        
        const syncResult = await req.db.query(syncQuery);
        const ultimaSync = syncResult.rows[0]?.ultima_sync;
        ...
        console.log(`[PRESUPUESTOS-BACK] Última sincronización: ${ultimaSync}`);
        ...
        res.json({
            ...
            estadisticas: {
                // Campos que espera el frontend
                total_registros: parseInt(stats.total_presupuestos || 0),
                total_categorias: parseInt(stats.total_agentes || 0), // Usando agentes como categorías
                monto_total: parseFloat(articulosStats.valor_total_estimado || 0),
                ultima_sincronizacion: ultimaSync,
                
                // Campos adicionales para información completa
                total_presupuestos: parseInt(stats.total_presupuestos || 0),
                total_clientes: parseInt(stats.total_clientes || 0),
                total_agentes: parseInt(stats.total_agentes || 0),
