...
import { modalIngredienteHTML, inicializarModalIngrediente, abrirModalNuevoIngrediente } from './components/modalIngrediente.js';
...
// ==========================================
// TICKET #017 / #018 / #020: ALTA IN-PLACE DE NUEVO INGREDIENTE DESDE RECETAS
// ==========================================
let modalIngredienteInicializado = false;

document.addEventListener('click', async (e) => {
    // Botón "Nuevo" (Abrir modal)
    const btnNuevoIngrediente = e.target.closest('#btn-nuevo-ingrediente');
    
    // Verificamos que estamos en el contexto del modal de recetas
    if (btnNuevoIngrediente && btnNuevoIngrediente.closest('#modal-receta')) {
        ...
        if (contenedor && !modalIngredienteInicializado) {
            try {
                contenedor.innerHTML = modalIngredienteHTML;
                await inicializarModalIngrediente(async (nuevoIngrediente) => {
                    // Callback de guardado exitoso
                    
                    // Recargar ingredientes en memoria global para el buscador
                    if (typeof window.cargarIngredientesDisponibles === 'function') {
                        await window.cargarIngredientesDisponibles();
                    } else if (typeof cargarIngredientesDisponibles === 'function') {
                        await cargarIngredientesDisponibles();
                    }
                ...
                modalIngredienteInicializado = true;
            } catch (error) {
                console.error('❌ Error crítico al inicializar el modal de ingredientes:', error);
                alert('Hubo un problema al cargar el formulario de ingredientes. Intente recargar la página.');
                return; // Cortar la ejecución si falla la inicialización
            }
        }
        
        // Abrir modal con la API de modalIngrediente.js (solo si se inicializó correctamente)
        if (modalIngredienteInicializado) {
