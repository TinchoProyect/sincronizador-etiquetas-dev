...
async function verificarTablaIngredientes() {
    try {
        // 1. Verificar si la tabla existe
        console.log('1. Verificando existencia de la tabla ingredientes...');
        const existsQuery = `
            SELECT EXISTS (
        ...
        if (tableExists.rows[0].exists) {
            ...
            const countQuery = 'SELECT COUNT(*) FROM public.ingredientes;';
            ...
            if (count.rows[0].count > 0) {
                ...
                const sampleQuery = 'SELECT * FROM public.ingredientes LIMIT 5;';
