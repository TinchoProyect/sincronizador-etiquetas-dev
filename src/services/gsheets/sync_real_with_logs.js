...
function mapTwoSheetsToPresupuestos(presupuestosData, detallesData, config) {
    ...
    for (let i = 0; i < presupuestosData.rows.length; i++) {
        ...
        try {
            ...
            if (!id_presupuesto_ext || !id_cliente) {
                ...
                console.log(`[PRESUPUESTOS-BACK] Valores: id_presupuesto_ext="${id_presupuesto_ext}", id_cliente="${id_cliente}"`);
                continue;
            }
    ...
    console.log('[PRESUPUESTOS-BACK] Encabezados DetallesPresupuestos:', detallesData.headers);
    console.log('[PRESUPUESTOS-BACK] ¿Encabezados válidos para DetallesPresupuestos?', detallesData.headers.length > 0);
    ...
    for (let i = 0; i < detallesData.rows.length; i++) {
        ...
        try {
            ...
            if (!id_presupuesto || !articulo) {
                ...
                console.log(`[PRESUPUESTOS-BACK] Valores: id_presupuesto="${id_presupuesto}", articulo="${articulo}"`);
                continue;
            }
            ...
            if (presupuestosMap.has(presupuestoKey)) {
                ...
                console.log(`[PRESUPUESTOS-BACK] ⚠️ Detalle fila ${i + 2}: Presupuesto ${presupuestoKey} no encontrado, omitiendo detalle`);
                console.log(`[PRESUPUESTOS-BACK] Presupuestos disponibles:`, Array.from(presupuestosMap.keys()));
            }
    ...
    console.log('[PRESUPUESTOS-BACK] Primeros 2 presupuestos mapeados:', presupuestosArray.slice(0, 2));
