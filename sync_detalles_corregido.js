...
/**
 * Sincronización de detalles usando tabla de mapeo como fuente de verdad - CORREGIDA
 * @param {string} idPresupuesto - ID del presupuesto a sincronizar
 * @param {Object} config - Configuración de Sheets
 * @param {Object} db - Conexión a base de datos
 */
async function sincronizarDetallesConMapeoCorregida(idPresupuesto, config, db) {
    console.log(`🔄 [SYNC-CORREGIDO] === SINCRONIZACIÓN CORREGIDA - PRESUPUESTO ${idPresupuesto} ===\n`);
    ...
    try {
        ...
        console.log('📋 [SYNC-CORREGIDO] 1. Leyendo datos locales...');
        ...
        if (rsLocal.rows.length === 0) {
            console.log('✅ [SYNC-CORREGIDO] No hay detalles para sincronizar');
            return resumen;
        }
        
        // 2. CONSULTAR TABLA DE MAPEO
        console.log('📋 [SYNC-CORREGIDO] 2. Consultando tabla de mapeo...');
        ...
        console.log('📋 [SYNC-CORREGIDO] 3. Leyendo datos de Sheets...');
        ...
        if (existingSheetData.rows) {
            existingSheetData.rows.forEach((row, index) => {
                // CORRECCIÓN: Usar acceso por nombre de header
                const idDetallePresupuesto = String(row.IDDetallePresupuesto || '').trim();
                const idPresupuestoSheet = String(row.IdPresupuesto || '').trim();
                const articuloSheet = String(row.Articulo || '').trim();
        ...
        console.log('📋 [SYNC-CORREGIDO] 4. Procesando detalles...');
        ...
        // 5. RESUMEN FINAL
        console.log('\n📊 [SYNC-CORREGIDO] 5. RESUMEN FINAL:');
        ...
        console.log('\n✅ [SYNC-CORREGIDO] Sincronización completada');
        
    } catch (error) {
        console.error('❌ [SYNC-CORREGIDO] Error:', error.message);
...
async function ejecutarSincronizacionCorregida() {
    try {
        const config = { 
        ...
        const resultado = await sincronizarDetallesConMapeoCorregida('06c76584', config, pool);
...
if (require.main === module) {
    ejecutarSincronizacionCorregida();
}

module.exports = {
    sincronizarDetallesConMapeoCorregida
};
