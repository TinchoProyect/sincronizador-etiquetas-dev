    ...
    try {
        const query = `
            ...
            WHERE mm.tipo_movimiento = 'ENVIO_TRATAMIENTO'
              ...
              -- Evitamos re-achicar cosas que ya se mandaron como "1" 
        ...
        console.log(\`✅ Data Patch Aplicado. Registros de Tratamiento históricos purgados: \${res.rowCount}\`);
        if(res.rowCount>0) console.table(res.rows);
    } catch(e) {
        console.error('Error aplicando parche a Mantenimiento:', e.message);
    } finally {
        pool.end();
    }
}
